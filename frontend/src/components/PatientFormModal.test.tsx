...
describe('PatientFormModal', () => {
  ...
  it('shows "Edit Patient" title when a patientId is provided', () => {
    mockApiFetch.mockResolvedValueOnce({ ok: true, json: () => Promise.resolve(existingPatient) } as Response)
    render(<PatientFormModal open={true} patientId={existingPatient.id} onClose={onClose} onSuccess={onSuccess} />)
    expect(screen.getByText('Edit Patient')).toBeInTheDocument()
  })

  it('pre-populates all form fields with existing patient data in edit mode', async () => {
    mockApiFetch.mockResolvedValueOnce({ ok: true, json: () => Promise.resolve(existingPatient) } as Response)
    render(<PatientFormModal open={true} patientId={existingPatient.id} onClose={onClose} onSuccess={onSuccess} />)
    expect(await screen.findByDisplayValue('Jane')).toBeInTheDocument()
    expect(screen.getByDisplayValue('Smith')).toBeInTheDocument()
    expect(screen.getByDisplayValue('Dr.')).toBeInTheDocument()
    expect(screen.getByDisplayValue('1980-06-15')).toBeInTheDocument()
  })
  ...
  it('sends a PUT request with the patient id in edit mode', async () => {
    mockApiFetch
      .mockResolvedValueOnce({ ok: true, json: () => Promise.resolve(existingPatient) } as Response)
